 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
   <!-- Content Header (Page header) -->
    <section class="content-header">
            <h1>
              Panel de control
              <small>Descargas</small>
            </h1><br>
              <ol class="breadcrumb">
               <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
              </ol>
    </section>

          <!-- Main content -->
    <section class="content">
       


           <div class="box-footer">

            <h2>Formatos Descargables</h2>
            <hr>
              <ul class="mailbox-attachments clearfix">
                <li>
                   <a href="<?=base_url()?>src/formatos/formato53.xls"
                  <span class="mailbox-attachment-icon bg-green"><i class="fa fa-file-excel-o"></i></span>
                    </a>
                  <div class="mailbox-attachment-info">
                    <a href="<?=base_url()?>src/formatos/formato53.xls" class="mailbox-attachment-name"></i> Formato 5.3</a>
                        <span class="mailbox-attachment-size">
                          25.0 KB
                          <a href="<?=base_url()?>src/formatos/formato56.xls" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
                        </span>
                  </div>
                </li>
                <li>
                   <a href="<?=base_url()?>src/formatos/formato56.xls"
                  <span class="mailbox-attachment-icon bg-green"><i class="fa fa-file-excel-o"></i></span>
                </a>
                  <div class="mailbox-attachment-info">
                    <a href="<?=base_url()?>src/formatos/formato56.xls" class="mailbox-attachment-name">Formato 5.6</a>
                        <span class="mailbox-attachment-size">
                          35.5 KB
                          <a href="<?=base_url()?>src/formatos/formato56.xls" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
                        </span>
                  </div>
                </li>
                <li>
                   <a href="<?=base_url()?>src/formatos/formato57.xls"
                  <span class="mailbox-attachment-icon bg-green"><i class="fa fa-file-excel-o"></i></span>
                </a>
                  <div class="mailbox-attachment-info">
                    <a href="<?=base_url()?>src/formatos/formato57.xls" class="mailbox-attachment-name">Formato 5.7</a>
                        <span class="mailbox-attachment-size">
                          35.5 KB
                          <a href="<?=base_url()?>src/formatos/formato57.xls" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
                        </span>
                  </div>
                </li>
               
              </ul>
            </div>

               </section>
             </div>

              <!-- /.box-body -->
