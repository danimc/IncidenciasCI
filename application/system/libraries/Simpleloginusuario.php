 ...
 * 
 */
class SimpleLoginUsuario
{
	var $CI;
	var $user_table = 'usuarios u';

	function SimpleLoginUsuario()
	{
	...
	function create($user = '', $password = '', $auto_login = true) {
		...
		if ($query->num_rows() > 0) {
			...
			if($auto_login) {		
				...
				//Set session data
				$this->CI->session->set_userdata(array('id' => $user_id,'correo' => $user));
				
				//Set logged_in to true
				$this->CI->session->set_userdata(array('usuario_login' => true));		
	...
	function login($user = '', $password = '') {
		...
		if($this->CI->session->userdata('correo') == $user) {
			//User is already logged in.
			return false;
		}
		
		//Check against user table
                $this->CI->db->select("u.id as id,correo,password,nombre,apellidos,telefonos");
		$this->CI->db->where('correo', $user);
